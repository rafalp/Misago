...
This hook wraps a standard Misago function used to post-process post content after saving.
...
```python
def custom_post_process_post_content_filter(
    action: PostProcessPostContentHookAction, post: Post
):
    ...
```
...
```python
def post_process_post_content_action(post: Post):
    ...
```

Misago function used to post-process post content or the next filter function from another plugin.
