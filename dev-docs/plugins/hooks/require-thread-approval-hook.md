...
```python
def custom_require_thread_approval_filter(
    action: RequireThreadApprovalHookAction, state: 'ThreadStartState'
) -> bool:
    ...
```
...
```python
def require_thread_approval_action(state: 'ThreadStartState') -> bool:
    ...
```
