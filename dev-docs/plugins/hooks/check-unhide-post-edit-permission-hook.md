...
This hook wraps the standard Misago function used to check whether a user has permission to unhide a hidden post edit. Raises Django's `PermissionDenied` if they don't.
...
```python
def custom_check_unhide_post_edit_permission_filter(
...
```python
def check_unhide_post_edit_permission_action(
    permissions: 'UserPermissionsProxy', post_edit: PostEdit
) -> None:
    ...
```

Misago function used to check if a user has permission to unhide a hidden post edit. Raises Django's `PermissionDenied` if they don't.
