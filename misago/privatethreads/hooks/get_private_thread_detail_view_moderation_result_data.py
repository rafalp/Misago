...
class GetPrivateThreadDetailViewModerationResultDataHook(
    FilterHook[
        GetPrivateThreadDetailViewModerationResultDataHookAction,
        GetPrivateThreadDetailViewModerationResultDataHookFilter,
    ]
):
    """
    This hook wraps the standard function that Misago uses to get
    the template context data for the moderation result in the private thread
    detail view.
...
get_private_thread_detail_view_moderation_result_data_hook = (
    GetPrivateThreadDetailViewModerationResultDataHook(cache=False)
)
