...
class UpdateAstMetadataPostsHookAction(Protocol):
    """
    A standard Misago function used to update metadata with posts from the database,
...
class UpdateAstMetadataPostsHookFilter(Protocol):
    """
    A function implemented by a plugin that can be registered in this hook.

    # Arguments

    ## `action: UpdateAstMetadataPostsHookAction`
...
class UpdateAstMetadataPostsHook(
    FilterHook[UpdateAstMetadataPostsHookAction, UpdateAstMetadataPostsHookFilter]
):
    """
    This hook wraps the standard function that Misago uses to update metadata
    with posts from the database.
...
update_ast_metadata_posts_hook = UpdateAstMetadataPostsHook()
