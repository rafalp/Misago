...
def test_post_paginator_without_orphans_returns_next_page_first_item(
    thread_reply_factory, thread
):
    queryset = thread.post_set.order_by("id")

    # First item page
    paginator = PostPaginator(queryset, 5, 0)
...
def test_post_paginator_with_orphans_returns_next_page_first_item(
    thread_reply_factory, thread
):
    queryset = thread.post_set.order_by("id")

    # First item page
    paginator = PostPaginator(queryset, 5, 2)
    assert paginator.page(1).next_page_first_item is None
...
def test_post_paginator_without_orphans_calculates_item_page(
    thread_reply_factory, thread
):
    # First item page
    paginator = PostPaginator(thread.post_set, 5, 0)
