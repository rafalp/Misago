...
def test_replied_thread_is_watched_with_email_notifications(user, old_thread):
    user.watch_replied_threads = ThreadNotifications.SITE_AND_EMAIL
    user.save()

    watch_replied_thread(user, old_thread)

    watched_thread = WatchedThread.objects.get(user=user, thread=old_thread)
    assert watched_thread.send_emails


def test_replied_thread_is_watched_without_email_notifications(user, old_thread):
...
def test_replied_thread_is_not_watched_if_option_is_disabled(user, old_thread):
