...
def test_thread_post_like_view_likes_post_in_htmx(user_client, user, thread, post):
    response = user_client.post(
        reverse(
            "misago:thread-post-like",
            kwargs={"thread_id": thread.id, "slug": thread.slug, "post_id": post.id},
...
def test_thread_post_like_view_does_nothing_for_already_liked_post_in_htmx(
    user_client, user, thread, post
):
    like = like_post(post, user)
...
def test_thread_post_like_view_returns_error_404_if_thread_doesnt_exist(
    user_client, user
):
    response = user_client.post(
        reverse(
...
def test_thread_post_like_view_returns_error_404_if_thread_post_doesnt_exist(
...
def test_thread_post_like_view_returns_error_404_if_post_doesnt_exist_in_thread(
...
def test_thread_post_like_view_returns_error_404_if_user_has_no_category_permission(
    user_client, user, thread, post
):
...
def test_thread_post_like_view_returns_error_404_if_user_has_no_thread_permission(
    user_client, user, thread, post
):
...
def test_thread_post_like_view_returns_error_404_if_user_has_no_post_permission(
    user_client, user, thread, post
):
...
def test_thread_post_like_view_returns_error_403_if_user_is_anonymous(
    ...
    assert_contains(response, "You can&#x27;t like posts.", status_code=403)
...
def test_thread_post_like_view_returns_error_403_if_user_cant_like_posts(
    ...
    assert_contains(response, "You can&#x27;t like this post.", status_code=403)
...
def test_thread_post_like_view_returns_error_404_if_post_is_in_private_thread(
