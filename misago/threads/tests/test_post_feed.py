...
from ..postfeed import PostFeed, PrivateThreadPostFeed, ThreadPostFeed
...
def test_post_feed_returns_post_data(request_factory, user, thread, post):
    request = request_factory(user)

    post_feed = PostFeed(request, thread, [post])
    feed_data = post_feed.get_context_data()

    assert feed_data["template_name"] == post_feed.template_name
    assert feed_data["items"][0]["template_name"] == post_feed.post_template_name
    assert feed_data["items"][0]["post"] == post


def test_post_feed_sets_posters_in_post_data(
    request_factory, user, other_user, thread, other_user_reply
):
    request = request_factory(user)

    post_feed = PostFeed(request, thread, [other_user_reply])
    feed_data = post_feed.get_context_data()
...
def test_post_feed_sets_rich_text_data_in_post_data(
...
def test_post_feed_sets_extra_post_attachments(
    request_factory, user, thread, other_user_reply, text_attachment
):
    text_attachment.associate_with_post(other_user_reply)
    text_attachment.save()
...
def test_post_feed_marks_post_as_animated(request_factory, user, thread, post, reply):
    request = request_factory(user)

    post_feed = PostFeed(request, thread, [post, reply])
    post_feed.set_animated_posts([post.id])
    feed_data = post_feed.get_context_data()
...
def test_post_feed_marks_post_as_new(request_factory, user, thread, post, reply):
    request = request_factory(user)

    post_feed = PostFeed(request, thread, [post, reply])
    post_feed.set_unread_posts([post.id])
    feed_data = post_feed.get_context_data()
...
def test_thread_post_feed_marks_post_as_editable(
    request_factory, user, thread, user_reply, reply
):
    request = request_factory(user)

    post_feed = ThreadPostFeed(request, thread, [user_reply, reply])
    feed_data = post_feed.get_context_data()
...
def test_thread_post_feed_marks_original_post_as_editable(
    request_factory, user, user_thread, reply
):
    request = request_factory(user)

    post_feed = ThreadPostFeed(request, user_thread, [user_thread.first_post, reply])
    feed_data = post_feed.get_context_data()
...
def test_thread_post_feed_marks_original_post_as_thread_editable(
    request_factory, user, user_thread, reply
):
    request = request_factory(user)
...
def test_thread_post_feed_marks_post_as_editable_by_moderator(
...
def test_thread_post_feed_marks_original_post_as_editable_by_moderator(
...
def test_thread_post_feed_marks_original_post_as_thread_editable_by_moderator(
    request_factory, moderator, user_thread, reply
):
    request = request_factory(moderator)
...
def test_post_feed_returns_thread_update_data(
    ...
    post_feed = PostFeed(request, thread, [post], [thread_update])
    feed_data = post_feed.get_context_data()
...
def test_thread_post_feed_doesnt_mark_thread_update_as_hidable_by_user(
    ...
    post_feed = ThreadPostFeed(request, thread, [post], [thread_update])
    feed_data = post_feed.get_context_data()
...
def test_thread_post_feed_doesnt_mark_thread_update_as_unhideable_by_user(
    ...
    post_feed = ThreadPostFeed(request, thread, [post], [hidden_thread_update])
    feed_data = post_feed.get_context_data()
