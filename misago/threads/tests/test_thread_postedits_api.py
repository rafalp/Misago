...
class ThreadPostGetEditTests(ThreadPostEditsApiTestCase):
    ...
    def test_empty_edit_id(self):
        ...
        response = self.client.get(f"{self.api_link}?edit=")
        self.assertEqual(response.status_code, 404)

    def test_invalid_edit_id(self):
        """api handles invalid edit in querystring"""
        response = self.client.get(f"{self.api_link}?edit=dsa67d8sa68")
        self.assertEqual(response.status_code, 404)

    def test_nonexistant_edit_id(self):
        """api handles nonexistant edit in querystring"""
        response = self.client.get(f"{self.api_link}?edit=1321")
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_middle_edit(self):
        ...
        response = self.client.get(f"{self.api_link}?edit={edits[1].id}")
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_first_edit(self):
        ...
        response = self.client.get(f"{self.api_link}?edit={edits[0].id}")
        self.assertEqual(response.status_code, 200)
...
class ThreadPostPostEditTests(ThreadPostEditsApiTestCase):
    ...
    def test_empty_edit_id(self):
        """api handles empty edit in querystring"""
        response = self.client.post(f"{self.api_link}?edit=")
        self.assertEqual(response.status_code, 404)

    @patch_category_acl({"can_edit_posts": 2})
    def test_invalid_edit_id(self):
        """api handles invalid edit in querystring"""
        response = self.client.post(f"{self.api_link}?edit=dsa67d8sa68")
        self.assertEqual(response.status_code, 404)

    @patch_category_acl({"can_edit_posts": 2})
    def test_nonexistant_edit_id(self):
        """api handles nonexistant edit in querystring"""
        response = self.client.post(f"{self.api_link}?edit=1321")
        self.assertEqual(response.status_code, 404)
    ...
    def test_anonymous(self):
        ...
        response = self.client.post(f"{self.api_link}?edit={self.edits[0].id}")
        self.assertEqual(response.status_code, 403)
