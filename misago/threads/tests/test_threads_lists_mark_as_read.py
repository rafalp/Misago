...
def test_site_threads_list_mark_as_read_marks_all_categories_as_read(
    ...
    response = user_client.post(
        reverse("misago:threads"), {"mark_as_read": "true", "confirm": "true"}
    )
    assert response.status_code == 302
    assert response["location"] == reverse("misago:threads")
...
def test_private_threads_list_mark_as_read_marks_category_as_read(
    user_client, user, private_threads_category, thread
):
    ReadThread.objects.create(
        user=user,
