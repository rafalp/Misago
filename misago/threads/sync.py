...
async def get_thread_stats(thread_id: int, base_query: Optional[Query] = None) -> dict:
...
async def sync_thread(
    thread: Thread, base_query: Optional[Query] = None
) -> Tuple[Thread, dict]:
    stats = await get_thread_stats(thread.id, base_query)
    return (await thread.update(**stats)), stats


async def sync_thread_by_id(thread_id: int, base_query: Optional[Query] = None) -> dict:
