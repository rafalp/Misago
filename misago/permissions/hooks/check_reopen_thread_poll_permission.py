...
class CheckReopenThreadPollPermissionHookAction(Protocol):
    """
    A standard Misago function used to check if the user has permission to reopen
    a closed thread poll. Raises Django's `PermissionDenied` exception with an
    error message if the user lacks permission.
...
class CheckReopenThreadPollPermissionHookFilter(Protocol):
    """
...
class CheckReopenThreadPollPermissionHook(
    FilterHook[
        CheckReopenThreadPollPermissionHookAction,
        CheckReopenThreadPollPermissionHookFilter,
    ]
):
    """
    This hook allows plugins to replace or extend the permission check for the
    "can reopen closed thread poll" permission.
...
check_reopen_thread_poll_permission_hook = CheckReopenThreadPollPermissionHook()
