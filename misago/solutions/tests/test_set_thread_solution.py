from ..solutions import set_thread_solution


def test_set_thread_solution_sets_thread_solution_by_user(
...
def test_set_thread_solution_sets_thread_solution_by_deleted_user(
    other_user, thread, other_user_reply
):
    set_thread_solution(thread, other_user_reply, "DeletedUser")
...
def test_set_thread_solution_sets_deleted_user_thread_solution_by_user(
    user, thread, reply
):
    set_thread_solution(thread, reply, user)
...
def test_set_thread_solution_doesnt_save_changes_if_commit_is_false(
    django_assert_num_queries, user, other_user, thread, other_user_reply
):
    with django_assert_num_queries(0):
        set_thread_solution(thread, other_user_reply, user, commit=False)
