...
class TokenizeSearchQueryHookAction(Protocol):
    """
    ...
    def __call__(self, query: str) -> Optional[list["Token"]]: ...


class TokenizeSearchQueryHookFilter(Protocol):
    """
...
class TokenizeSearchQueryHook(
    FilterHook[
        TokenizeSearchQueryHookAction,
        TokenizeSearchQueryHookFilter,
    ]
):
    """
    This hook wraps a standard Misago function used to parse a search query
    used for forum search to a token stream.

    It returns a list of 'Token' objects that the next parsing step will convert
    into a `SearchQuery`.
...
tokenize_search_query_hook = TokenizeSearchQueryHook()
