...
from ...threads.move import move_thread as move_thread_and_posts
from ...types import (
    AsyncValidator,
    GraphQLContext,
    MoveThreadInput,
...
@move_thread_mutation.field("editThreadTitle")
...
async def resolve_move_thread(
    ...
    thread = await move_thread_hook.call_action(move_thread, info.context, cleaned_data)
...
async def move_thread(context: GraphQLContext, cleaned_data: MoveThreadInput) -> Thread:
    ...
    thread = await move_thread_and_posts(thread, cleaned_data["category"])
