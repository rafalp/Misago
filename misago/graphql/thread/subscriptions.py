...
from ariadne_graphql_modules import SubscriptionType, gql

from ...categories.get import get_category_by_id
from ...categories.models import Category
from ...pubsub.threads import threads_updates
from ..args import InvalidArgumentError, clean_id_arg


class ThreadSubscriptions(SubscriptionType):
    __schema__ = gql(
        """
        type Subscription {
            threads(category: ID): ID!
        }
        """
    )

    @classmethod
    async def subscribe_threads(cls, *_, category: Optional[str] = None):
        category_obj = None
        ...
        async for event in threads_updates():
            ...
            if event_category_id not in categories_map:
                categories_map[
                    event_category_id
                ] = await cls.is_event_in_subscribed_category(
    ...
    async def is_event_in_subscribed_category(
        cls, category: Optional[Category], event_category_id: int
    ) -> bool:
    ...
    @classmethod
    def resolve_threads(
        cls, obj, *_, category: Optional[str] = None  # pylint: disable=unused-argument
    ):
        return obj["id"]
