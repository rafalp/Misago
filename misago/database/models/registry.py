...
class MapperRegistry:
    ...
    def set_mapping(self, table: TableClause, model: Type | Type[dict]):
        self.tables[table.name] = table
        self.mappings[table.name] = model
...
def register_model(table: TableClause, name: Optional[str] = None):
    def register(model: Type[Model]):
        fin_name = name or model.__name__

        mapper_registry.set_mapping(table, model)
        mapper_registry.set_model(fin_name, model)

        model.query = mapper_registry.query_table(table)
        model.table = table

        if issubclass(model, Model):
            create_model_exceptions(fin_name, model, table)
