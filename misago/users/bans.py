...
def _set_user_ban_cache(user, cache_versions):
    ...
    try:
        ...
        ban_cache.team_reason = user_ban.team_reason
    except Ban.DoesNotExist:
        ban_cache.ban = None
        ban_cache.expires_on = None
        ban_cache.user_message = None
        ban_cache.team_reason = None
...
def ban_user(user, user_message=None, team_reason=None, length=None, expires_on=None):
...
def ban_ip(ip, user_message=None, team_reason=None, length=None, expires_on=None):
    if not expires_on and length:
        expires_on = timezone.now() + timedelta(**length)
