...
def test_getting_user_by_username_is_not_doing_fuzzy_matching(db):
    User.objects.create_user("User", "test@example.com", profile_fields={})
    with pytest.raises(User.DoesNotExist):
...
def test_getting_user_by_email_supports_diacritics(db):
    user = User.objects.create_user("User", "łóć@łexĄmple.com", profile_fields={})
...
def test_combined_getter_handles_username_slug_and_email_collision(db):
    email_match = User.objects.create_user("Bob", "test@test.test", profile_fields={})
    slug_match = User.objects.create_user("TestTestTest", "other@test.com", profile_fields={})
