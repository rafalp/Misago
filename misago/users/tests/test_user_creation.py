...
def test_user_is_created(db):
    user = User.objects.create_user("User", "test@example.com", profile_fields={})
    assert user.pk
    assert user.joined_on


def test_user_is_created_with_username_and_slug(db):
    user = User.objects.create_user("UserName", "test@example.com", profile_fields={})
...
def test_user_is_created_with_useable_password(db):
    password = "password"
    user = User.objects.create_user("UserUserame", "test@example.com", password, profile_fields={})
...
def test_user_is_created_with_custom_rank(db):
    ...
    user = User.objects.create_user("User", "test@example.com", rank=rank, profile_fields={})
...
def test_user_is_created_with_diacritics_in_email(db):
    ...
    user = User.objects.create_user("UserName", email, profile_fields={})
    assert user.email == email


def test_creating_user_without_username_raises_value_error(db):
    with pytest.raises(ValueError):
        User.objects.create_user("", "test@example.com", profile_fields={})


def test_creating_user_without_email_raises_value_error(db):
    with pytest.raises(ValueError):
        User.objects.create_user("User", "", profile_fields={})


def test_create_superuser(db):
    user = User.objects.create_superuser("User", "test@example.com", profile_fields={})
...
def test_creating_superuser_without_staff_status_raises_value_error(db):
    with pytest.raises(ValueError):
        User.objects.create_superuser("User", "test@example.com", is_staff=False, profile_fields={})


def test_creating_superuser_without_superuser_status_raises_value_error(db):
    with pytest.raises(ValueError):
        User.objects.create_superuser("User", "test@example.com", is_superuser=False, profile_fields={})
