...
def test_tracking_script_is_not_included_if_tracking_id_is_not_set(db, user_client):
    response = user_client.get("/")
    assert_not_contains(response, "googletagmanager.com/gtag/js")


@override_dynamic_settings(google_tracking_id="UA-TEST")
def test_tracking_script_is_included_if_tracking_id_is_not_set(db, user_client):
    response = user_client.get("/")
